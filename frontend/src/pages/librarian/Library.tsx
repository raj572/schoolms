...
import { Book, BookOpen, Clock, AlertCircle, RefreshCw, Plus, Search, Edit, RotateCcw, Trash2 } from "lucide-react";
import { useEffect, useState } from "react";
import { useAuthStore } from "@/store/useAuthStore";
import { getLibraryStatistics, getAllBooks, getActiveIssues, getAllIssues, getOverdueIssues, searchBooks, deleteBook, returnBook, updateBook } from "@/services/libraryApiService";
...
const LibrarianLibrary = () => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      ...
      try {
        ...
        // Fetch all issues for history tab
        const allIssuesResponse = await getAllIssues(schoolId);
  ...
  const handleDeleteBook = async (bookId: number) => {
    ...
    try {
      ...
      if (response.status) {
        ...
        // Refresh books list
        if (authUser?.school_id) {
          const booksResponse = await getAllBooks(Number(authUser.school_id));
          if (booksResponse.status && booksResponse.data) {
            setBooks(booksResponse.data);
          }
          // Refresh statistics
          const statsResponse = await getLibraryStatistics(Number(authUser.school_id));
          if (statsResponse.status && statsResponse.data) {
            setStatistics(statsResponse.data);
          }
        }
  ...
  const handleRefresh = async () => {
    ...
    try {
      ...
      const [statsResponse, booksResponse, issuesResponse, allIssuesResponse] = await Promise.all([
